...
static void write_flg(t_form mod, int *astn);
...
char *write_str(t_arrs *arrs, char *str, size_t offset, t_form mod)
{
    ...
    if (check_excpt(mod.end, str[offset + 1]))
        return (NULL);
    if (mod.end != '\0')
        write_flg(mod, arrs->astn);
    if (!write_spcf(offset, mod, arrs, str))
        return (NULL);
    return (str);
}
...
static char  *check_extval(int (*ext)[2], char *str, int i)
{
    ...
    {
        numstr = ft_substr((const char *)str, ext[i][0], ext[i][1] - ext[i][0] + 1);
        if (!numstr)
            return (NULL);
